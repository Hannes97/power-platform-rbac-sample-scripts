...
$body = @{
    principalObjectId = $config.EntraUserObjectId
    principalType     = "User"
...
try {
    ...
    if ($_.Exception.Response.StatusCode -eq [System.Net.HttpStatusCode]::Conflict) {
        Write-Host "Role assignment already exists for user $($config.EntraUserObjectId) on environment group $($config.EnvironmentGroupId) — nothing to do." -ForegroundColor Yellow
    }
    else {
        throw
    }
