...
$servicePrincipalHeaders = & "$PSScriptRoot\..\auth.ps1" $config
...
$environmentsResponse = Invoke-RestMethod -Method GET -Uri "https://api.powerplatform.com/environmentmanagement/environments?`$filter=environmentGroupId%20eq%20'$($config.EnvironmentGroupId)'&api-version=2024-10-01" -Headers $servicePrincipalHeaders
$environments = $environmentsResponse.value

Write-Host "Found $($environments.Count) environments"

# Authenticate and obtain an access token from admin user
Connect-AzAccount
...
$adminUserHeaders = @{ 'Authorization' = 'Bearer ' + $accessToken }
$adminUserHeaders.Add('Content-Type', 'application/json')
...
foreach ($env in $environments) {
    ...
    $existingAssignments = Invoke-RestMethod -Method GET -Uri $assignmentUri -Headers $adminUserHeaders
    ...
    $body = @{
        ...
        scope             = "/tenants/$($config.TenantId)/environments/$environmentId"
    } | ConvertTo-Json
    ...
    if ($DryRun) {
        ...
        try {
            Invoke-RestMethod -Method POST -Uri $assignmentUri -Headers $adminUserHeaders -Body $body
        } catch {
